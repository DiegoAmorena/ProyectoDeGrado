...
class CourseScraper:
    def __init__(self, logger, url="https://open.fing.edu.uy/courses/", db_path="../DB/CoursesNames/"):
        self.url = url
    ...
    def fetch_courses(self):
        if self.should_run():
            ...
            if response.status_code == 200:
                ...
                self.logger.log_message("Course acronyms and names have been written to 'course_acronyms.txt' and 'course_names.txt'.")
                self.update_timestamp()
            else:
                self.logger.log_message(f"Failed to retrieve the page. Status code: {response.status_code}")
        else:
            self.logger.log_message("CourseScraper skipped as it was already run within the last week.")
